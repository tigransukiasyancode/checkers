...
void  PossibleMoves::getAllForIndex( Board* board, int index , list<Move>* moves){
	if(Figures::isChecker(board->get(index))){
		getCheckerMoves(index,board,moves);
		getCheckerJumps(index,board,moves);
	}else{
		getKingMoves(index,board,moves);	
		getKingJumps(index,board,moves);
	}
}
...
void  PossibleMoves::detectCheckerJumpsPaths(int index 
	...
	for(int i =0 ; i<length ; ++i){
			...
			&& !flags[cap_ind] ) 
		{
			...
			path->push_back(Move::CapturingPosition(cap_ind,to));
			flags[cap_ind] = 1;
			flags[index] = 1;
...
void PossibleMoves::detectKingJumpPaths(int index 
	...
	BoardPositions::R_SEQ move_sequences =  *_move_sequences;
	BoardPositions::R_SEQ_LEN lengths = * _lengths;
	int last_captured = capture_path->empty()? -1 : capture_path->back().position;
	for(int i = 0 ; i<length ; ++i){
		...
		if( sequence[k] != last_captured && Figures::isOponents(figure , board->get(sequence[k]))){
			...
			for(; k >= 0 && ( board->isFree(sequence[k]) || flags[sequence[k]] ) ; --k){
				...
				flags[cap_ind] = 1;
				flags[index] = 1;
				capture_path->push_back(Move::CapturingPosition(cap_ind,to));
